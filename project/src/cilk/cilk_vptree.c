...
#pragma grainsize (numberOfPoints / 8)
...
void buildVPTree(vptree *parentTree, double **points, double *distances, int64_t dimension, int64_t numberOfPoints){
    ...
    if(parentTree->inner->stop - parentTree->inner->start + 1 >= numberOfPoints / 32768) {
...
int main(int argc, char **argv){
    unsigned int numWorkers = __cilkrts_get_nworkers();
    printf("The workers are: %u\n", numWorkers);
