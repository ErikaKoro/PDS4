...
### 5.1. Sequential
Simply run `make run_sequential` in the [*PDS4/project*](/project) directory in order to run the executable for sequential code that creates the vantage point tree. The executable files will be created in the `BUILD_DIR` directory along with the `object` files.
